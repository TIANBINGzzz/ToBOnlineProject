...
// status=11
function UserComplaint ({ getUser }) {
  ...
  function RemoveNotification(id, idx){
    fetch(`http://localhost:8080/notification/process/${id}?status=14&trigger=true`, {
      method:'PUT',
      headers: {
        'content-type': 'application/json',
        'Authorization': `${token}`
  ...
  return (
    <Background ref={projectOverviewRef} id='complaint_scrollable_container'>
      <InfiniteScroll_wrapper>
        <InfiniteScroll
          ...
          scrollableTarget="complaint_scrollable_container"
        >
          {notifications.length !== 0 &&
            notifications.map((notification, index1) => (
              <Card key={notification.id}>
                ...
                />
                <TitleContainer>
                  <Project_title onClick={getUser(notification.userId)}>{notification.userName}</Project_title>
                </TitleContainer>
